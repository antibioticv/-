...
class sm2Encrypt:                                  #SM2加解密
    def encrypt(self,info):        #加密
        Ciphertext = sm2_crypt.encrypt(info.encode(encoding="utf-8"))
        Ciphertext = b64encode(encode_info).decode()  # 将二进制bytes通过base64编码
        return Ciphertext
    def decrypt(self, info):
        Text = b64decode(info.encode())  # 通过base64解码成二进制bytes
        Text = sm2_crypt.decrypt(decode_info).decode(encoding="utf-8")
        return Text
...
sm2 = sm2Encrypt()
Key="ghsjdkfushdhskdh"
data=input('please enter the data:')
def PGP():
    aes=AesEncryption(Key)
    C=aes.encrypt(data)  # 调用加密函数
    print("加密的消息为：",C)
    Message=aes.decrypt(C) # 调用解密函数
    print("解密后的消息为：",Message)
PGP()
